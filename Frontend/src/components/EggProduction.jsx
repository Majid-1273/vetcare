...
function EggProduction() {
  ...
  const [eggProductionData, setEggProductionData] = useState([
    { id: 1, date: '2025-04-01', morning: 590, midmorning: 425, afternoon: 485, totalEggs: 1500, brokenEggs: 15, productionPercent: 87.5, selected: true },
    { id: 2, date: '2025-04-02', morning: 610, midmorning: 400, afternoon: 490, totalEggs: 1500, brokenEggs: 18, productionPercent: 87.5, selected: true },
    { id: 3, date: '2025-04-03', morning: 580, midmorning: 430, afternoon: 470, totalEggs: 1480, brokenEggs: 12, productionPercent: 86.4, selected: false },
    { id: 4, date: '2025-04-04', morning: 600, midmorning: 410, afternoon: 480, totalEggs: 1490, brokenEggs: 14, productionPercent: 87, selected: false }
  ]);
  ...
  const areAllSelected = eggProductionData.every(row => row.selected);

  // Toggle all row selections
  const toggleAllRowsSelection = () => {
    const newSelectState = !areAllSelected;
    setEggProductionData(eggProductionData.map(row => ({ ...row, selected: newSelectState })));
  };
    ...
    setEggProductionData(eggProductionData.map(row => {
      if (row.id === id) {
        return {
          ...
          morning: editFormData.morning,
          midmorning: editFormData.midmorning,
          afternoon: editFormData.afternoon,
          totalEggs: totalEggs,
          brokenEggs: editFormData.brokenEggs,
  ...
  const handleAddRecord = () => {
    ...
    const newRecord = {
      ...
      morning: newRecordData.morning,
      midmorning: newRecordData.midmorning,
      afternoon: newRecordData.afternoon,
      totalEggs: totalEggs,
      brokenEggs: newRecordData.brokenEggs,
    ...
    <div className="p-6 bg-gray-50">
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          ...
          >
            <span className="mr-2 text-lg">+</span> Add Egg Production Record
          </button>
        </div>

        {/* Egg Production data table */}
                ...
                <tr key={row.id} className={`${row.selected ? 'bg-green-50' : 'bg-white'} hover:bg-gray-50 transition-colors`}>
                  ...
                  {editingRow === row.id ? (
                    ...
                    <>
                      ...
                      <td className="py-2 px-4">
                        <input
                          type="number"
                          name="afternoon"
                          value={editFormData.afternoon}
                          onChange={handleEditFormChange}
                          className="border rounded px-2 py-1 w-full focus:outline-none focus:ring-2 focus:ring-green-500"
                        />
                      </td>
                      <td className="py-2 px-4 font-medium">
                        {editFormData.morning + editFormData.midmorning + editFormData.afternoon}
                      </td>
                      <td className="py-2 px-4">
                        <input
                          type="number"
                      ...
                      <td className="py-2 px-4 font-medium">
                        {(((editFormData.morning + editFormData.midmorning + editFormData.afternoon) / 1720) * 100).toFixed(1)}%
                      </td>
                    ...
                    <>
                      ...
                      <td className="py-4 px-4">{row.morning}</td>
                      <td className="py-4 px-4">{row.midmorning}</td>
                      <td className="py-4 px-4">{row.afternoon}</td>
                      <td className="py-4 px-4 font-medium">{row.totalEggs}</td>
                      <td className="py-4 px-4 text-red-600">{row.brokenEggs}</td>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      type="number"
                      name="midmorning"
                      value={newRecordData.midmorning}
                      onChange={handleNewRecordChange}
                      className="w-full border rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-500"
                    />
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      type="number"
                      name="brokenEggs"
                      value={newRecordData.brokenEggs}
                      onChange={handleNewRecordChange}
                      className="w-full border rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-500"
                    />
                  </div>
                ...
                <div>
                  ...
                  <div className="bg-gray-50 p-3 rounded-md">
                    <div className="flex justify-between mb-1">
                      ...
                      <span className="font-medium">{newRecordData.morning + newRecordData.midmorning + newRecordData.afternoon}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Production %:</span>
                      <span className="font-medium">
                        {(((newRecordData.morning + newRecordData.midmorning + newRecordData.afternoon) / 1720) * 100).toFixed(1)}%
                      </span>
                    </div>
                  </div>
                </div>
